 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\flv_media_sink.cpp
...
bool FlvMediaSink::init() { return true; }

FlvMediaSink::~FlvMediaSink() { spdlog::info("destroy FlvMediaSink"); }
...
bool FlvMediaSink::on_source_codec_ready(std::shared_ptr<Codec> video_codec,
                                         std::shared_ptr<Codec> audio_codec) {
    bool ret = ready_cb_(video_codec, audio_codec);
    return ret;
}

void FlvMediaSink::set_source_codec_ready_cb(
    const std::function<bool(std::shared_ptr<Codec> video_codec, std::shared_ptr<Codec> audio_codec)>
        &ready_cb) {
    ready_cb_ = ready_cb;
}

void FlvMediaSink::on_flv_tag(
    const std::function<boost::asio::awaitable<bool>(std::vector<std::shared_ptr<FlvTag>> &msgs)> &cb) {
    cb_ = cb;
}
