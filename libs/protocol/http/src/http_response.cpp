...
std::shared_ptr<SocketInterface> HttpResponse::get_conn() { return conn_; }

void HttpResponse::add_header(const std::string &k, const std::string &v) { headers_[k] = v; }
...
const std::string &HttpResponse::get_version() const { return version_; }

int32_t HttpResponse::get_status_code() const { return status_code_; }

const std::string &HttpResponse::get_msg() const { return msg_; }
...
boost::asio::awaitable<bool> HttpResponse::recv_http_header() {
    ...
    while (1) {
        ...
        auto recv_size = co_await conn_->recv_some((uint8_t *)recv_buf_.get() + recv_buf_size_,
                                                   buffer_size_ - recv_buf_size_);
        if (recv_size < 0) {
            co_return false;
        }
...
boost::asio::awaitable<void> HttpResponse::cycle_recv_body(
    const std::function<boost::asio::awaitable<int32_t>(const std::string_view &)> &cb) {
    ...
    if (content_len > 0) {
        while (true) {
            ...
            while (total_consumed < recv_buf_size_) {
                consumed = co_await cb(
                    std::string_view(recv_buf_.get() + total_consumed, recv_buf_size_ - total_consumed));
                if (consumed < 0) {
                    co_return;
                } else if (consumed == 0) {
                    break;
        ...
        if (is_chunked) {
            ...
            while (1) {
                ...
                while (recv_chunk_buf_size_ > 0 && total_chunk_consumed < recv_chunk_buf_size_) {
                    consumed = co_await cb(std::string_view(recv_chunk_buf_.get() + total_chunk_consumed,
                ...
                if (total_chunk_consumed > 0) {
                    recv_chunk_buf_size_ -= total_chunk_consumed;
                    memmove(recv_chunk_buf_.get(), recv_chunk_buf_.get() + total_chunk_consumed,
                            recv_chunk_buf_size_);
                }
...
ThreadWorker *HttpResponse::get_worker() { return conn_->get_worker(); }
