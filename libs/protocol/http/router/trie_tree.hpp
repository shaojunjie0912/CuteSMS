...
public:
    ...
    // 获取一条路由
    std::optional<HANDLER> get_route(const std::string& path,
...
private:
    ...
    bool add_route(const std::string_view& path, TrieNode& node, const HANDLER& handler) {
        ...
        if (c == ':') {    // 如果是冒号，则匹配到/,.或者末尾为止
            ...
            if (j == path.size()) {  // 到了末尾 /:stream
                ...
                bool ret =
                    add_route(std::string_view(path.data() + j, path.size() - j), *param_child_node, handler);
                if (!ret) {
                    return false;
                }
                child_node->pchildrens[param_name] = param_child_node;
            ...
            if (child_node) {  // 存在
                ...
                child_node->c = c;
                bool ret =
                    add_route(std::string_view(path.data() + 1, path.size() - 1), *child_node, handler);
                if (!ret) {
                    return false;
                }
    ...
    // 获取一条从node开始的路由
    std::optional<HANDLER> get_route(const std::string_view& path, TrieNode& node,
                                     std::unordered_map<std::string, std::string>& params) {
        if (path.size() <= 0) {
        ...
        if (child) {
            ...
            for (auto it = child->pchildrens.begin(); it != child->pchildrens.end(); it++) {
                handler =
                    get_route(std::string_view(path.data() + j, path.size() - j), *(it->second), params);
                if (handler) {
                    // it->first = param_name
