 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\server\webrtc\dtls_boringssl\dtls_boringssl_session.h
...
public:
    ...
    enum DtlsState { NEW = 1, CONNECTING, CONNECTED, FAILED, CLOSED };
...
public:
    ...
    boost::asio::awaitable<bool> process_dtls_packet(uint8_t *data, size_t len, UdpSocket *sock,
                                                     const boost::asio::ip::udp::endpoint &remote_ep);
    void on_handshake_done(
        const std::function<void(SRTPProtectionProfile profile, const std::string &srtp_recv_key,
                                 const std::string &srtp_send_key)> &cb) {
        handshake_done_cb_ = cb;
    }

private:
    boost::asio::awaitable<void> send_pending_outgoing_dtls_packet();
    std::function<void(SRTPProtectionProfile profile, const std::string &srtp_recv_key,
