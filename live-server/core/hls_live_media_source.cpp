 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\hls_live_media_source.cpp
...
HlsLiveMediaSource::HlsLiveMediaSource(ThreadWorker *worker, std::weak_ptr<StreamSession> session,
                                       std::shared_ptr<PublishApp> app)
    : MediaSource("hls", session, app, worker) {}

HlsLiveMediaSource::~HlsLiveMediaSource() {}

bool HlsLiveMediaSource::init() { return true; }
...
std::shared_ptr<MediaBridge> HlsLiveMediaSource::get_or_create_bridge(const std::string &id,
                                                                      std::shared_ptr<PublishApp> app,
