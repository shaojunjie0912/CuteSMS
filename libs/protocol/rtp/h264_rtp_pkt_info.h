...
public:
    ...
    H264_RTP_HEADER_TYPE get_type() const { return type; }

    bool is_single_nalu() const {
        return type >= H264_RTP_PAYLOAD_SINGLE_NALU_START && type <= H264_RTP_PAYLOAD_SINGLE_NALU_END;
    }

    bool is_stap_a() const { return type == H264_RTP_PAYLOAD_STAP_A; }

    uint8_t get_nalu_type() const { return nalu_type; }

    uint8_t is_start_fu() const { return start_bit; }

    uint8_t is_end_fu() const { return end_bit; }
