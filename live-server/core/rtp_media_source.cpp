 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\rtp_media_source.cpp
...
RtpMediaSource::RtpMediaSource(const std::string &media_type, std::weak_ptr<StreamSession> session,
                               std::shared_ptr<PublishApp> app, ThreadWorker *worker)
    : MediaSource(media_type, session, app, worker) {}

bool RtpMediaSource::init() { return true; }

RtpMediaSource::~RtpMediaSource() {}

boost::asio::awaitable<bool> RtpMediaSource::on_audio_packets(
...
boost::asio::awaitable<bool> RtpMediaSource::on_video_packets(
...
std::shared_ptr<MediaBridge> RtpMediaSource::get_or_create_bridge(const std::string &id,
