...
App::App(const std::string &domain, const std::string &app_name)
    : domain_name_(domain), app_name_(app_name) {}

App::~App() { uninit(); }

void App::init() {
    try {
        app_logger_ = spdlog::daily_logger_mt(domain_name_ + "/" + app_name_,
                                              "logs/" + domain_name_ + "/" + app_name_ + ".log", 00, 00);
...
spdlog::logger *App::get_logger() { return app_logger_ptr_; }

void App::update_conf(std::shared_ptr<AppConfig> app_config) { app_conf_ = app_config; }

std::shared_ptr<AppConfig> App::get_conf() { return app_conf_; }