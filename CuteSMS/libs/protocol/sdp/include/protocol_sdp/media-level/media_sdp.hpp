...
#include "../attribute/common/control.hpp"
#include "../attribute/common/dir.hpp"
#include "../attribute/common/maxptime.hpp"
#include "../attribute/common/recvonly.hpp"
#include "../attribute/common/rtcp_mux.h"
#include "../attribute/common/rtpmap.h"
#include "../attribute/common/sendonly.hpp"
#include "../attribute/common/sendrecv.hpp"
#include "../dtls/fingerprint.h"
#include "../dtls/setup.h"
#include "../ice/ice_options.h"
#include "../ice/ice_pwd.h"
#include "../ice/ice_ufrag.h"
#include "../session-level/candidate.h"
#include "../session-level/connection_info.hpp"
#include "../webrtc/extmap.hpp"
...
// 1E:4C:4A:70:97:1A:23:6D:CB:96:D4:25:A4:24:35:16:F3:2D:D1:10:40:7E:04:B3:47:23:B5:8D:0D:AC:8D:50
// a=setup:actpass
// a=mid:0
...
public:
    ...
    void set_media(const std::string &val) { media = val; }

    uint16_t get_port() const { return port; }

    void set_port(uint16_t val) { port = val; }

    uint16_t get_port_count() const { return port_count; }

    void set_port_count(uint16_t val) { port_count = val; }

    const std::string &get_proto() const { return proto; }

    void set_proto(const std::string &val) { proto = val; }

    const std::vector<uint16_t> &get_fmts() const { return fmts; }

    void add_fmt(uint16_t val) { fmts.push_back(val); }
    ...
    void set_fmts(const std::vector<uint16_t> &val) { fmts = val; }

    const std::optional<IceUfrag> &get_ice_ufrag() const { return ice_ufrag_; }

    void set_ice_ufrag(const IceUfrag &val) { ice_ufrag_ = val; }

    const std::optional<IcePwd> &get_ice_pwd() const { return ice_pwd_; }

    void set_ice_pwd(const IcePwd &val) { ice_pwd_ = val; }

    const std::optional<IceOption> get_ice_option() const { return ice_option_; }

    void set_ice_option(const IceOption &val) { ice_option_ = val; }

    void add_candidate(const Candidate &c) { candidates_.push_back(c); }

    const std::vector<Candidate> &get_candidates() const { return candidates_; }

    const std::vector<Extmap> &get_extmap() const { return ext_maps; }

    void set_extmap(const std::vector<Extmap> &val) { ext_maps = val; }

    void add_extmap(const Extmap &val) { ext_maps.push_back(val); }

    const std::optional<DirAttr> &get_dir() const { return dir; }

    void set_dir(const DirAttr &val) { dir = val; }

    const std::optional<MidAttr> &get_mid_attr() const { return mid; }

    void set_mid_attr(const MidAttr &val) { mid = val; }
    ...
    void set_connection_info(const ConnectionInfo &info) { connection_info = info; }

    void set_setup(const SetupAttr &val) { setup_ = val; }

    const std::optional<SetupAttr> &get_setup() const { return setup_; }

    void set_rtcp_mux(const RtcpMux &rtcp_mux) { rtcp_mux_ = rtcp_mux; }

    const std::unordered_map<uint32_t, Ssrc> &get_ssrcs() const { return ssrcs_; }
    ...
    void set_ssrc_group(const SsrcGroup &val) { ssrc_group_ = val; }

    std::optional<SsrcGroup> get_ssrc_group() const { return ssrc_group_; }

    void add_payload(const Payload &p) { payloads_.insert({p.get_pt(), p}); }
    std::string to_string() const;

    std::unordered_map<int, Payload> &get_payloads() { return payloads_; }
    ...
    void set_finger_print(const FingerPrint &fp) { fingerprint_ = fp; }

    std::optional<FingerPrint> &get_finger_print() { return fingerprint_; }

    const std::optional<Control> &get_control() const { return control; }

    void set_control(const Control &val) { control = val; }

    const BandWidth &get_bw() const { return bw_; }

    void set_bw(const BandWidth &bw) { bw_ = bw; }
...
private:
    ...
    //       （如果不遵从偶数RTP端口和奇数RTCP端口的规则，那么要用a=rtcp:指定RTCP端口） Applications that are
    //       requested to send media to a <port> that is odd and where the "a=rtcp:" is present MUST NOT
    //       subtract 1 from the RTP port: that is, they MUST send the RTP to the port indicated in <port> and
    //       send the RTCP to the port indicated in the "a=rtcp" attribute.
    ...
    //   transport-protocol_specific but format-independent are possible.
    uint16_t port;
    uint16_t port_count;
    std::string proto;
