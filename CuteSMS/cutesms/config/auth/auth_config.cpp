 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\config\source_config.cpp
...
int32_t AuthConfig::parse_check_config(const YAML::Node& node, const YAML::Node& parent_node) {
    ...
    std::string method_name =
        check_pattern.substr(left_bracket_pos + 1, right_bracket_pos - left_bracket_pos - 1);
    ...
    std::string param2 =
        check_pattern.substr(right_bracket_pos + 1, check_pattern.size() - right_bracket_pos - 1);
