    ...
    while (len > 0) {
        ...
        switch (t) {
            ...
            case STUN_ATTR_GOOG_NETWORK_INFO: {
                auto goog_network_info_attr =
                    std::unique_ptr<StunGoogNetworkInfoAttr>(new StunGoogNetworkInfoAttr);
            ...
            case STUN_ICE_ATTR_USE_CANDIDATE: {
                auto ice_use_candidate_attr =
                    std::unique_ptr<StunIceUseCandidateAttr>(new StunIceUseCandidateAttr);
            ...
            case STUN_ICE_ATTR_ICE_CONTROLLING: {
                auto ice_controlling_attr =
                    std::unique_ptr<StunIceControllingAttr>(new StunIceControllingAttr);
        ...
        /*
            ...
            all other attributes that follow MESSAGE-INTEGRITY.
        */
        if (msg_integrity_attr) {  // 如果解析到message integrity了，则最后只解析fingerprint了
            break;
        }
    }
...
int32_t StunMsg::encode(uint8_t *data, size_t len, bool add_message_integrity, const std::string &pwd,
                        bool add_finger_print) {
    ...
    if (add_message_integrity) {
        msg_integrity_attr = std::unique_ptr<StunMessageIntegrityAttr>(
            new StunMessageIntegrityAttr(data_start, header.length + 20, add_finger_print, pwd));
    ...
    if (add_finger_print) {
        fingerprint_attr =
            std::unique_ptr<StunFingerPrintAttr>(new StunFingerPrintAttr(data_start, data - data_start));
