...
struct StunMsg {
    ...
    void add_attr(std::unique_ptr<StunMsgAttr> attr) { attrs.emplace_back(std::move(attr)); }

    uint16_t type() { return header.type; }

    bool has_msg_integrity_attr() const { return msg_integrity_attr != nullptr; }

    bool has_finger_print() const { return fingerprint_attr != nullptr; }

    int32_t decode(uint8_t *data, size_t len);

    size_t size(bool add_message_integrity = false, bool add_finger_print = false);

    virtual int32_t encode(uint8_t *data, size_t len, bool add_message_integrity = false,
                           const std::string &pwd = "", bool add_finger_print = false);

    bool check_msg_integrity(uint8_t *data, size_t len, const std::string &pwd);
    bool check_finger_print(uint8_t *data, size_t len);

    const std::optional<StunUsernameAttr> &get_username_attr() const { return username_attr; }

    void set_username_attr(const StunUsernameAttr &username) { username_attr = username; }
};
};  // namespace cutesms