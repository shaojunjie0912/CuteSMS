...
bool Mp4MediaSink::init() { return true; }
...
bool Mp4MediaSink::recv_video_init_segment(std::shared_ptr<Mp4Segment> mp4_seg) {
    ...
    boost::asio::co_spawn(
        ...
        [this, self, mp4_seg]() -> boost::asio::awaitable<void> {
...
void Mp4MediaSink::set_video_init_segment_cb(
    const std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> seg)> &cb) {
    video_init_seg_cb_ = cb;
}

void Mp4MediaSink::set_audio_init_segment_cb(
    const std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> seg)> &cb) {
    audio_init_seg_cb_ = cb;
}

void Mp4MediaSink::set_video_mp4_segment_cb(
    const std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> seg)> &cb) {
    video_seg_cb_ = cb;
}

void Mp4MediaSink::set_audio_mp4_segment_cb(
    const std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> seg)> &cb) {
    audio_seg_cb_ = cb;
}
