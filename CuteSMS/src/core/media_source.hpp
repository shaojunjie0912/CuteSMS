...
public:
    MediaSource(const std::string &media_type, std::weak_ptr<StreamSession> session,
    ...
    virtual std::shared_ptr<Recorder> get_or_create_recorder(const std::string &type,
                                                             std::shared_ptr<PublishApp> app);

    const std::string &get_media_type() const { return media_type_; }

    void set_media_type(const std::string &v) { media_type_ = v; }
    ...
    void set_source_info(const std::string &domain, const std::string &app_name,
    ...
    std::shared_ptr<Codec> get_video_codec() { return video_codec_; }

    std::shared_ptr<Codec> get_audio_codec() { return audio_codec_; }

    bool is_origin() const { return is_origin_; }

    void set_origin(bool v) { is_origin_ = v; }
...
protected:
    ...
    std::unordered_map<std::string, std::shared_ptr<MediaBridge>>
        bridges_;  // 桥接类型的sink，意味着这只是一个桥，没人的时候要拆掉，而且桥可以转换格式之类的
