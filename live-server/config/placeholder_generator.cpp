...
int32_t PlaceHolderGenerator::parse_string_to_placeholders(
    const YAML::Node& node, const std::string& unformatte_str,
...
int32_t PlaceHolderGenerator::parse_method_param(const YAML::Node& node, std::shared_ptr<Param> param_holder,
...
std::shared_ptr<PlaceHolder> PlaceHolderGenerator::gen_holder(
    const YAML::Node& node, std::string& seg, bool is_string_holder,
    std::map<std::string, std::shared_ptr<Param>>& params) {
    if (is_string_holder) {
...
std::shared_ptr<Param> PlaceHolderGenerator::recursive_search_param_node(
    const YAML::Node& node, const std::string& param_name,
    std::map<std::string, std::shared_ptr<Param>>& params) {
    ...
    if (params_node.IsDefined()) {
        if (params_node.size() > 0) {
            for (YAML::const_iterator it = params_node.begin(); it != params_node.end(); it++) {
                ...
                std::string method_params_list =
                    param_pattern.substr(left_bracket_pos + 1, param_pattern.size() - left_bracket_pos - 2);
