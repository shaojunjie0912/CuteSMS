...
int32_t HEVCDecoderConfigurationRecord::encode(uint8_t *data, int32_t len) {
    ...
    *data =
        (constantFrameRate << 6) | (numTemporalLayers << 4) | (temporalIdNested << 2) | (lengthSizeMinusOne);
    data++;
    len--;
...
const std::string &HEVCDecoderConfigurationRecord::get_sps() { return sps_nalu_; }

const std::string &HEVCDecoderConfigurationRecord::get_pps() { return pps_nalu_; }

const std::string &HEVCDecoderConfigurationRecord::get_vps() { return vps_nalu_; }
}  // namespace cutesms