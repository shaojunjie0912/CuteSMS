 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\server\transcode\media_bridge.hpp
...
public:
    MediaBridge(ThreadWorker *worker, std::shared_ptr<PublishApp> app,
    ...
    virtual ~MediaBridge();
    virtual bool init() = 0;
    ThreadWorker *get_worker() const { return worker_; }

    std::shared_ptr<PublishApp> get_app() const { return publish_app_; }

    std::shared_ptr<MediaSource> get_media_source() { return source_; }

    std::shared_ptr<MediaSink> get_media_sink() { return sink_; }

    inline std::string &get_session_name() { return session_name_; }
