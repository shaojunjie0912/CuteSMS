 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\server\http\websocket_server_session.cpp
...
WebSocketServerSession::WebSocketServerSession(std::shared_ptr<SocketInterface> sock)
    : Session(sock->get_worker()), sock_(sock) {
...
void WebSocketServerSession::service() {
    ...
    boost::asio::co_spawn(
        ...
        [this, self]() -> boost::asio::awaitable<void> {
            // co_await sock_->cycle_recv([this](const char *buf, int32_t
            // len)->boost::asio::awaitable<std::pair<bool,int32_t>> {
