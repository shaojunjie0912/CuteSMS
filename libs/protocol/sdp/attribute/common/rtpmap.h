...
struct Rtpmap {
public:
    inline static std::string prefix{"a=rtpmap:"};
    bool parse(const std::string& line);
    uint32_t get_payload_type() const { return payload_type; }

    void set_payload_type(uint32_t val) { payload_type = val; }

    const std::string& get_encoding_name() const { return encoding_name; }

    void set_encoding_name(const std::string& val) { encoding_name = val; }

    uint32_t get_clock_rate() const { return clock_rate; }

    void set_clock_rate(uint32_t val) { clock_rate = val; }

    void add_encoding_param(const std::string& val) { encoding_params.push_back(val); }

    const std::vector<std::string>& get_encoding_params() const { return encoding_params; }

    void set_encoding_params(const std::vector<std::string>& params) { encoding_params = params; }
