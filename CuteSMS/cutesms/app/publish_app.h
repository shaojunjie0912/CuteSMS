...
public:
    int32_t on_create_source(const std::string& domain, const std::string& app_name,
                             const std::string& stream_name, std::shared_ptr<MediaSource> source);
    boost::asio::awaitable<std::shared_ptr<MediaSource>> find_media_source(
        std::shared_ptr<StreamSession> session) override;
    std::vector<std::shared_ptr<MediaSink>> create_push_streams(std::shared_ptr<MediaSource> source,
    ...
    virtual void on_destroy_source(const std::string& domain, const std::string& app_name,
                                   const std::string& stream_name, std::shared_ptr<MediaSource> source);
...
private:
    ...
    boost::asio::awaitable<Error> invoke_on_publish_http_callback(const HttpCallbackConfig& conf,
                                                                  std::shared_ptr<StreamSession> session);
    boost::asio::awaitable<Error> invoke_on_unpublish_http_callback(const HttpCallbackConfig& conf,
                                                                    std::shared_ptr<StreamSession> session);
};
};  // namespace cutesms