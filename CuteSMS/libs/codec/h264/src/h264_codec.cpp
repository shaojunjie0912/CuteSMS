...
std::shared_ptr<Payload> H264Codec::get_payload() { return sdp_payload_; }
...
bool H264Codec::get_wh(uint32_t& w, uint32_t& h) {
    ...
    w = width_;
    h = height_;
    if (sps_.vui_parameters_present_flag && sps_.vui_parameters.timing_info_present_flag &&
        sps_.vui_parameters.num_units_in_tick > 0) {
