...
namespace cutesms {
UdpSocket::UdpSocket(UdpSocketHandler *handler, std::unique_ptr<boost::asio::ip::udp::socket> sock)
    : handler_(handler), sock_(std::move(sock)) {}

UdpSocket::UdpSocket(std::unique_ptr<boost::asio::ip::udp::socket> sock) : sock_(std::move(sock)) {}

boost::asio::awaitable<bool> UdpSocket::send_to(uint8_t *data, size_t len,
                                                const boost::asio::ip::udp::endpoint &remote_ep) {
    boost::system::error_code ec;
    auto size = co_await sock_->async_send_to(boost::asio::buffer(data, len), remote_ep,
...
boost::asio::awaitable<int32_t> UdpSocket::recv_from(uint8_t *data, size_t len,
                                                     boost::asio::ip::udp::endpoint &remote_ep) {
    boost::system::error_code ec;
    auto size =
        co_await sock_->async_receive_from(boost::asio::buffer(data, len), remote_ep,
                                           boost::asio::redirect_error(boost::asio::use_awaitable, ec));
    if (ec) {
        co_return -1;
    }
