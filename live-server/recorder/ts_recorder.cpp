...
TsRecorder::~TsRecorder() { CORE_DEBUG("destroy TsRecorder"); }
...
bool TsRecorder::init() {
    ...
    ts_media_sink_->on_close([this, self]() { close(); });

    record_start_time_ = time(NULL);
    ts_media_sink_->on_ts_segment([this,
                                   self](std::shared_ptr<TsSegment> ts_seg) -> boost::asio::awaitable<bool> {
        boost::system::error_code ec;
        std::string file_name =
            std::to_string(ts_seg->get_create_at()) + "_" + std::to_string(ts_seg->get_seqno()) + "_" +
            std::to_string(ts_seg->get_duration()) + "_" + std::to_string(ts_seg->get_start_pts()) + ".ts";
        std::string file_dir = Config::get_instance()->get_record_root_path() + "/" + domain_name_ + "/" +
                               app_name_ + "/" + stream_name_ + "/ts/" + std::to_string(record_start_time_);
        try {
            // 写入文件
            std::filesystem::create_directories(file_dir);

            std::ofstream ts_file(file_dir + "/" + file_name,
