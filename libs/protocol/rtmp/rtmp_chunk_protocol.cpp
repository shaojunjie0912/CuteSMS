...
boost::asio::awaitable<int32_t> RtmpChunkProtocol::cycle_recv_rtmp_message(
    const std::function<boost::asio::awaitable<bool>(std::shared_ptr<RtmpMessage>)> &recv_handler) {
...
boost::asio::awaitable<bool> RtmpChunkProtocol::send_rtmp_messages(
        ...
        while (left_size > 0) {
            ...
            if (prev_chunk) {
                if (prev_chunk->chunk_message_header_.message_stream_id_ == rtmp_msg->message_stream_id_ &&
                    ...
                    prev_chunk->chunk_message_header_.message_length_ ==
                        (int32_t)rtmp_msg->get_using_data().size() &&
                    prev_chunk->chunk_message_header_.timestamp_delta_ ==
                        (rtmp_msg->timestamp_ - prev_chunk->chunk_message_header_.timestamp_)) {  // fmt3
                    fmt = RTMP_CHUNK_FMT_TYPE3;
                } else if (prev_chunk->chunk_message_header_.message_stream_id_ ==
                               rtmp_msg->message_stream_id_ &&
                           prev_chunk->chunk_message_header_.message_type_id_ == rtmp_msg->message_type_id_ &&
                           prev_chunk->chunk_message_header_.message_length_ ==
                               (int32_t)rtmp_msg->get_using_data().size()) {  // fmt2
                    fmt = RTMP_CHUNK_FMT_TYPE2;
                } else if (prev_chunk->chunk_message_header_.message_stream_id_ ==
                           rtmp_msg->message_stream_id_) {  // fmt1
            ...
            if (prev_chunk) {
                chunk->chunk_message_header_.timestamp_delta_ =
            ...
            send_sv_bufs_.push_back(
                boost::asio::const_buffer((char *)chunk_headers_[curr_chunk_header].get(), buf_pos));
            // 发送chunk data
            send_sv_bufs_.push_back(boost::asio::const_buffer(
                (char *)rtmp_msg->get_using_data().data() + cur_pos, this_chunk_payload_size));
            curr_chunk_header++;
...
boost::asio::awaitable<int32_t> RtmpChunkProtocol::process_recv_buffer() {
    ...
    if (fmt == RTMP_CHUNK_FMT_TYPE0) {
        ...
        chunk->chunk_message_header_.message_stream_id_ =
            prev_chunk->chunk_message_header_.message_stream_id_;
        // *chunk = *prev_chunk;
        if (left_size < 3) {
            co_return 0;
        }
        ...
        if (time_delta != 0xffffff) {
            chunk->chunk_message_header_.timestamp_ =
                prev_chunk->chunk_message_header_.timestamp_ + time_delta;
        ...
        chunk->chunk_message_header_.message_stream_id_ =
            prev_chunk->chunk_message_header_.message_stream_id_;
        chunk->chunk_message_header_.timestamp_ =
            prev_chunk->chunk_message_header_.timestamp_ + prev_chunk->chunk_message_header_.timestamp_delta_;
    ...
    // 处理chunk的payload
    int32_t this_chunk_payload_size =
        std::min(in_chunk_size_, chunk->chunk_message_header_.message_length_ -
                                     (int32_t)chunk->rtmp_message_->get_using_data().size());
    if (left_size < this_chunk_payload_size) {
        co_return 0;
    }

    memcpy((void *)chunk->rtmp_message_->get_unuse_data().data(), recv_buffer_ + buf_pos,
           this_chunk_payload_size);
    chunk->rtmp_message_->inc_used_bytes(this_chunk_payload_size);
    buf_pos += this_chunk_payload_size;
    left_size -= this_chunk_payload_size;

    // if we get a rtmp message
    if ((int32_t)chunk->rtmp_message_->get_using_data().size() ==
        chunk->chunk_message_header_.message_length_) {
        ...
        if (!ret) {
            spdlog::error("process rtmp message failed, close connection, type:{}",
                          (int32_t)chunk->rtmp_message_->message_type_id_);
            co_return -20;
        }
...
boost::asio::awaitable<bool> RtmpChunkProtocol::handle_protocol_control_message(
