 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\codec\hevc\hevc_codec.hpp
...
public:
    ...
    const std::string& get_sps_nalu() const { return sps_nalu_; }

    const std::string& get_pps_nalu() const { return pps_nalu_; }

    const std::string& get_vps_nalu() const { return vps_nalu_; }

    const std::string& get_sps_rbsp() const { return sps_rbsp_; }

    const std::string& get_pps_rbsp() const { return pps_rbsp_; }

    const std::string& get_vps() const { return vps_rbsp_; }
