...
DtlsCert::~DtlsCert() {
    // once you've successfully called EVP_PKEY_assign_RSA() you must not call RSA_free() on the underlying
    // key or a double-free may result. if (rsa_)
    // {
    //     RSA_free(rsa_);
    //     rsa_ = nullptr;
    // }
...
bool DtlsCert::create_cert() {
    ...
    // const u_char company[] = "cutesms, PLC";
    ...
    // key 写入文件，调试用
    std::unique_ptr<BIO, void (*)(BIO *)> keyFile{BIO_new_file("./cutesms.rsa", "wb"), BIO_free_all};
