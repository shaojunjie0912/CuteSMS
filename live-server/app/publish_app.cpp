...
PublishApp::PublishApp(const std::string& domain_name, const std::string& app_name)
    : App(domain_name, app_name) {
    is_publish_app_ = true;
}

PublishApp::~PublishApp() {}

boost::asio::awaitable<std::shared_ptr<MediaSource>> PublishApp::find_media_source(
    ...
    if (origin_pull_config->get_protocol() == "http-flv") {
        ...
        auto http_flv_client_session = std::make_shared<HttpFlvClientSession>(
            std::static_pointer_cast<PublishApp>(shared_from_this()), session->get_worker(),
            get_domain_name(), get_app_name(), session->get_stream_name());
        ...
        auto url = origin_pull_config->gen_url(session);
        std::shared_ptr<RtmpPlayClientSession> rtmp_play_client_session =
            std::make_shared<RtmpPlayClientSession>(std::static_pointer_cast<PublishApp>(shared_from_this()),
                                                    session->get_worker(), get_domain_name(), get_app_name(),
...
int32_t PublishApp::on_create_source(const std::string& domain, const std::string& app_name,
    ...
    if (source->is_origin()) {  // 只有是源流（真实客户端推送的流，才算，还有种流，是通过中转拉取,不算源流）
        ...
        for (auto& t : record_types) {
            ...
            auto recorder =
                source->get_or_create_recorder(t, std::static_pointer_cast<PublishApp>(shared_from_this()));
            if (recorder) {
...
void PublishApp::on_destroy_source(const std::string& domain, const std::string& app_name,
                                   const std::string& stream_name, std::shared_ptr<MediaSource> source) {
    (void)domain;
    (void)app_name;
    (void)stream_name;
...
std::vector<std::shared_ptr<MediaSink>> PublishApp::create_push_streams(
    std::shared_ptr<MediaSource> source, std::shared_ptr<StreamSession> session) {
...
boost::asio::awaitable<Error> PublishApp::invoke_on_publish_http_callback(
    const HttpCallbackConfig& conf, std::shared_ptr<StreamSession> session) {
    ...
    std::string msg;
    // todo :
    // 这里session已经在函数hold住了，lambda没必要传值，但是传值会导致内存异常，不知道为什么，还需要研究下
    co_await resp->cycle_recv_body(
        [this, &session, &code, &msg](const std::string_view& recv_data) -> boost::asio::awaitable<int32_t> {
            JSONCPP_STRING errs;
            Json::CharReaderBuilder readerBuilder;
            std::unique_ptr<Json::CharReader> const jsonReader(readerBuilder.newCharReader());
            Json::Value root;
            if (!jsonReader->parse((char*)recv_data.data(), (char*)recv_data.data() + recv_data.size(), &root,
                                   &errs)) {
                co_return 0;
            }
...
boost::asio::awaitable<Error> PublishApp::invoke_on_unpublish_http_callback(
    const HttpCallbackConfig& conf, std::shared_ptr<StreamSession> session) {
    auto url = conf.gen_url(session);
    std::string protocol;
    std::string domain;
