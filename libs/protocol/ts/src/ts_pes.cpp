...
static inline int64_t ff_parse_pes_pts(const char *buf) {
    return (int64_t)(*buf & 0x0e) << 29 | (AV_RB16(buf + 1) >> 1) << 15 | AV_RB16(buf + 3) >> 1;
}
...
int32_t PESPacketHeader::decode(const char *data, int32_t len) {
    ...
    if (stream_id != TsPESStreamIdProgramStreamMap && stream_id != TsPESStreamIdPaddingStream &&
        stream_id != TsPESStreamIdPrivateStream2 && stream_id != TsPESStreamIdEcmStream &&
        stream_id != TsPESStreamIdEmmStream && stream_id != TsPESStreamIdProgramStreamDirectory &&
        stream_id != TsPESStreamIdDsmccStream && stream_id != TsPESStreamIdH2221TypeE) {
        if (len < 1) {
            return 0;
        }
        ...
        //  CORE_INFO("PES_packet_data1.size():{}, stuffing_bytes:{}, header_consumed:{}",
        //  PES_packet_data.size(), stuffing_bytes, header_consumed); data += PES_packet_data.size(); len -=
        //  PES_packet_data.size();
    } else if (stream_id == TsPESStreamIdProgramStreamMap || stream_id == TsPESStreamIdPrivateStream2 ||
               stream_id == TsPESStreamIdEcmStream || stream_id == TsPESStreamIdEmmStream ||
               stream_id == TsPESStreamIdProgramStreamDirectory || stream_id == TsPESStreamIdDsmccStream ||
