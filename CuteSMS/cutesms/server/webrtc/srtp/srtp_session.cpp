...
bool SRTPSession::init(SRTPProtectionProfile profile, const std::string &recv_key,
    ...
    if ((int)recv_key.size() != srtp_policy_.rtp.cipher_key_len) {
        spdlog::error("recv_key was not match, len:{}, expected:{}", recv_key.size(),
                      srtp_policy_.rtp.cipher_key_len);
    }

    if ((int)send_key.size() != srtp_policy_.rtp.cipher_key_len) {
        spdlog::error("send_key was not match, len:{}, expected:{}", send_key.size(),
                      srtp_policy_.rtp.cipher_key_len);
    };
