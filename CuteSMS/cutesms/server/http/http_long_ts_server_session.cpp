...
HttpLongTsServerSession::HttpLongTsServerSession(std::shared_ptr<HttpRequest> http_req,
...
void HttpLongTsServerSession::service() {
    ...
    boost::asio::co_spawn(
        ...
        [this, self]() -> boost::asio::awaitable<void> {
            ...
            auto endpoint = this->get_param("cutesms-endpoint");
            ...
            if (!source) {  // todo : reply 404
                ...
                if (source->get_media_type() != "ts") {
                    ts_bridge = source->get_or_create_bridge(source->get_media_type() + "-ts", publish_app,
                                                             stream_name_);
                    if (!ts_bridge) {
                        http_response_->add_header("Connection", "close");
                        http_response_->add_header("Content-Length", "0");
                ...
                ts_media_sink_->on_pes_pkts([this](const std::vector<std::shared_ptr<PESPacket>>& pkts)
                    ...
                    co_await send_funcs_channel_.async_send(
                        ...
                        std::bind(&HttpLongTsServerSession::send_ts_seg, this, pkts),
                        boost::asio::redirect_error(boost::asio::use_awaitable, ec));
                    if (ec) {
                        co_return false;
                    }
...
boost::asio::awaitable<bool> HttpLongTsServerSession::send_ts_seg(
