...
void RtspsServer::stop() { tls_server_->stop_listen(); }
...
void RtspsServer::on_socket_close(std::shared_ptr<SocketInterface> tls_socket) {
    std::shared_ptr<RtspServerSession> s =
        std::static_pointer_cast<RtspServerSession>(tls_socket->get_session());
...
boost::asio::awaitable<void> RtspsServer::on_udp_socket_recv(UdpSocket *sock, std::unique_ptr<uint8_t[]> data,
                                                             size_t len,
