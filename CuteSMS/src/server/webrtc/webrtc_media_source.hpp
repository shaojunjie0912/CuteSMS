...
public:
    WebRtcMediaSource(ThreadWorker* worker, std::weak_ptr<StreamSession> session,
    ...
    uint16_t get_video_pt() const { return video_pt_; }

    uint16_t get_audio_pt() const { return audio_pt_; }

    uint32_t get_video_ssrc() { return video_ssrc_; }

    uint32_t get_audio_ssrc() { return audio_ssrc_; }
    ...
    const Sdp& get_remote_sdp() const { return remote_sdp_; }

    std::shared_ptr<Sdp> get_play_offer_sdp() const { return play_offer_sdp_; }

    void set_play_offer_sdp(std::shared_ptr<Sdp> sdp);
    void set_audio_pt(uint8_t v) { audio_pt_ = v; }

    void set_video_pt(uint8_t v) { video_pt_ = v; }

    void set_audio_ssrc(uint32_t v) { audio_ssrc_ = v; }
