...
int32_t CertManager::add_cert(const std::string &domain, const std::string &key_file,
                              const std::string &cert_file) {
    SSL_CTX *ssl_ctx = SSL_CTX_new(TLSv1_2_method());
    std::shared_ptr<SSL_CTX> ssl_ctx_ptr =
        std::shared_ptr<SSL_CTX>(ssl_ctx, [](SSL_CTX *ssl_ctx) { SSL_CTX_free(ssl_ctx); });

    if (!SSL_CTX_use_PrivateKey_file(
            ssl_ctx,
            (Utils::get_bin_path() + top_conf_.get_cert_root() + "/" + domain + "/" + key_file).c_str(),
            SSL_FILETYPE_PEM)) {
        return -1;
    }
    ...
    if (!SSL_CTX_use_certificate_file(
            ssl_ctx,
            (Utils::get_bin_path() + top_conf_.get_cert_root() + "/" + domain + "/" + cert_file).c_str(),
            SSL_FILETYPE_PEM)) {
        return -1;
    }
