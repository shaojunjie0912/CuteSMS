...
std::shared_ptr<OpusDecoder> OpusCodec::create_decoder(int32_t fs, int32_t channels) {
    int error;
    decoder_ = std::shared_ptr<OpusDecoder>(opus_decoder_create(fs, channels, &error),
                                            [](OpusDecoder *decoder) { opus_decoder_destroy(decoder); });
...
std::shared_ptr<Payload> OpusCodec::get_payload() { return sdp_payload_; }
}  // namespace cutesms