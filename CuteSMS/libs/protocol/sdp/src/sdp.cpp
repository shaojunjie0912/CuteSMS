...
int32_t Sdp::parse(const std::string &sdp) {
    ...
    size_t i = 3;
    while (!boost::starts_with(lines[i], MediaSdp::prefix)) {  // 在m=之前，都是session的属性
...
int Sdp::get_version() { return protocol_version_.get_version(); }

void Sdp::set_version(int v) { protocol_version_.set_version(v); }
