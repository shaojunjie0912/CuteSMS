...
HttpLongMp4ServerSession::HttpLongMp4ServerSession(std::shared_ptr<HttpRequest> http_req,
                                                   std::shared_ptr<HttpResponse> http_resp)
    : StreamSession(http_resp->get_worker()),
      send_funcs_channel_(get_worker()->get_io_context(), 2048),
      wg_(get_worker()) {
    ...
    boost::asio::co_spawn(
        ...
        [this, self]() -> boost::asio::awaitable<void> {
            ...
            if (source->get_media_type() != "mp4") {
                auto mp4_bridge = source->get_or_create_bridge(source->get_media_type() + "-mp4", publish_app,
                                                               stream_name_);
                if (!mp4_bridge) {
                    http_response_->add_header("Connection", "close");
                    http_response_->add_header("Content-Length", "0");
            ...
            mp4_media_sink_->set_audio_init_segment_cb(
                [this, self](std::shared_ptr<Mp4Segment> seg) -> boost::asio::awaitable<bool> {
                    ...
                    co_await send_funcs_channel_.async_send(
                        ...
                        std::bind(&HttpLongMp4ServerSession::send_fmp4_seg, this, seg),
                        boost::asio::redirect_error(boost::asio::use_awaitable, ec));
                    if (ec) {
                        co_return false;
                    }
