...
struct TAGDATA {
    ...
    const std::string_view &get_payload() const { return payload; }
...
#define MAKE_FOURCC(ch0, ch1, ch2, ch3) \
    (ch3) | (uint32_t(ch2) << 8) | (uint32_t(ch1) << 16) | (uint32_t(ch0) << 24)
struct VideoTagHeader {
    ...
    int32_t size_ = -1;  // 大小
public:
    bool is_key_frame() { return frame_type == KeyFrame; }

    bool is_seq_header();
    CodecID get_codec_id() { return codec_id; }
