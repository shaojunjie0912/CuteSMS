 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\server\http\https_server_base.hpp
...
#include "protocol_http/router/trie_tree.hpp"
#include "server_tls/tls_server.hpp"

namespace cutesms {
class WsConn;
class HttpsServerBase : public HttpRequestHandler,
                        public SocketInterfaceHandler,
                        public TlsServerNameHandler {
    using HTTPS_HANDLER = std::function<boost::asio::awaitable<void>(
        std::shared_ptr<HttpServerSession> session, std::shared_ptr<HttpRequest> req,
        std::shared_ptr<HttpResponse> resp)>;
    using WS_HANDLER = std::function<boost::asio::awaitable<void>(std::shared_ptr<HttpServerSession> session,
                                                                  std::shared_ptr<HttpRequest> req,
                                                                  std::shared_ptr<WsConn> ws_conn)>;
...
protected:
    ...
    boost::asio::awaitable<bool> on_new_request(std::shared_ptr<HttpServerSession> session,
                                                std::shared_ptr<HttpRequest> req,
                                                std::shared_ptr<HttpResponse> resp) override;
    // judge if websocket request
    bool is_websocket_req(std::shared_ptr<HttpRequest> req);
    boost::asio::awaitable<void> static_fs_handler(std::string root_path, std::string prefix,
