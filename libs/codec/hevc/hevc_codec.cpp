...
bool HevcCodec::is_ready() { return sps_nalu_.size() > 0 && pps_nalu_.size() > 0 && vps_nalu_.size() > 0; }
...
bool HevcCodec::gen_hevc_decoder_configuration_record() {
    ...
    if (!sps_pps_parsed_) {
        ...
        if (sps_.frame_cropping_flag) {
            ...
            height = ((2 - sps_.frame_mbs_only_flag) * height) - (sps_.frame_crop_top_offset * 2) -
                     (sps_.frame_crop_bottom_offset * 2);
        }
...
void HevcCodec::set_sps(const std::string& sps) {
    if (sps_nalu_.size() != sps.size() ||
        (sps_nalu_.size() > 0 && memcmp(sps_nalu_.data(), sps.data(), sps.size()) != 0)) {
...
void HevcCodec::set_pps(const std::string& pps) {
    if (pps_nalu_.size() != pps.size() ||
        (pps_nalu_.size() > 0 && memcmp(pps_nalu_.data(), pps.data(), pps.size()) != 0)) {
...
void HevcCodec::set_vps(const std::string& vps) {
    if (vps_nalu_.size() != vps.size() ||
        (vps_nalu_.size() > 0 && memcmp(vps_nalu_.data(), vps.data(), vps.size()) != 0)) {
...
void HevcCodec::set_sei_prefix(const std::string& sei_prefix) { sei_prefix_nalu_ = sei_prefix; }
