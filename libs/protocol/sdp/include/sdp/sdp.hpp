...
public:
    ...
    void set_origin(const Origin &origin) { origin_ = origin; }

    const Origin &get_origin() const { return origin_; }

    void set_session_name(const SessionName &session_name) { session_name_ = session_name; }

    SessionName &get_session_name() { return session_name_; }

    void set_time(const Timing &time) { time_ = time; }

    const Timing &get_time() { return time_; }

    void set_tool(const ToolAttr &tool) { tool_ = tool; }

    const ToolAttr &get_tool() { return tool_.value(); }

    void set_bundle(const BundleAttr &bundle) { bundle_attr_ = bundle; }

    const BundleAttr &get_bundle() { return bundle_attr_.value(); }

    std::vector<MediaSdp> &get_media_sdps() { return media_sdps_; }

    void set_media_sdps(const std::vector<MediaSdp> &media_sdps) { media_sdps_ = media_sdps; }

    void add_media_sdp(const MediaSdp &media_sdp) { media_sdps_.emplace_back(media_sdp); }

    void add_attr(const std::string &val) { attrs_.emplace_back(val); }
