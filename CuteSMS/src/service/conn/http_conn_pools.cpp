 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\conn_manager\http_conn_pools.cpp
...
void HttpConnPools::create_pool(
    ...
    boost::asio::co_spawn(
        ...
        [this, key, min_count, max_count, allocator]() -> boost::asio::awaitable<void> {
...
std::shared_ptr<ConnPool<HttpLongClient>> HttpConnPools::get_http_conn_pool(const std::string& domain,
                                                                            uint16_t port) {
    std::string key = domain + ":" + std::to_string(port);
    std::lock_guard<std::mutex> lck(http_conn_pools_mtx_);
    auto it = http_conn_pools_.find(key);
