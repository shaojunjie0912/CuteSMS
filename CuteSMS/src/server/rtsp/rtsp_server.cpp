...
RtspServer::RtspServer(const std::vector<ThreadWorker *> &workers)
    : UdpServer(workers), TcpServer(this, workers[0]) {}
...
void RtspServer::stop() { TcpServer::stop_listen(); }
...
void RtspServer::on_socket_close(std::shared_ptr<SocketInterface> tcp_socket) {
    std::shared_ptr<RtspServerSession> s =
        std::static_pointer_cast<RtspServerSession>(tcp_socket->get_session());
...
boost::asio::awaitable<void> RtspServer::on_udp_socket_recv(UdpSocket *sock, std::unique_ptr<uint8_t[]> data,
                                                            size_t len,
