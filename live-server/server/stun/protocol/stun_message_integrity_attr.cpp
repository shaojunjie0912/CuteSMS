...
#include "stun_msg.h"
StunMessageIntegrityAttr::StunMessageIntegrityAttr(uint8_t *data, size_t len, bool has_finger_print,
                                                   const std::string &pwd)
    : StunMsgAttr(STUN_ATTR_MESSAGE_INTEGRITY) {
...
size_t StunMessageIntegrityAttr::size() { return StunMsgAttr::size() + 20; }
...
bool StunMessageIntegrityAttr::check(StunMsg &stun_msg, uint8_t *data, size_t len, const std::string &pwd) {
    ...
    *(uint16_t *)(data + 2) = htons(content_len_with_message_integrity);
    HMAC_CTX *ctx = HMAC_CTX_new();
    HMAC_Init_ex(ctx, pwd.c_str(), pwd.size(), EVP_sha1(), NULL);
    HMAC_Update(ctx, (const unsigned char *)data,
                content_len_with_message_integrity + 20 -
                    24);  // 把头部长度加上，再减掉message integrity attr自己的长度
