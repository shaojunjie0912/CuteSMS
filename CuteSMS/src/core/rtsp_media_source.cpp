 ...
 * @Description:
 * @FilePath: \cutesms\cutesms\core\rtsp_media_source.cpp
...
RtspMediaSource::RtspMediaSource(ThreadWorker *worker, std::weak_ptr<StreamSession> s,
                                 std::shared_ptr<PublishApp> app)
    : RtpMediaSource("rtsp{rtp[es]}", s, app, worker) {}
...
std::shared_ptr<MediaBridge> RtspMediaSource::get_or_create_bridge(const std::string &id,
                                                                   std::shared_ptr<PublishApp> app,
