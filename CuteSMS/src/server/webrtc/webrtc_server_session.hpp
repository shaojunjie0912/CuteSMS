...
public:
    ...
    void set_close_handler(WebRtcServerSessionCloseHandler *close_handler) { close_handler_ = close_handler; }

    void service();
    void close() override;

    ThreadWorker *get_worker() { return worker_; }

    const std::string &get_local_ice_ufrag() const { return local_ice_ufrag_; }

    const std::string &get_local_ice_pwd() const { return local_ice_pwd_; }

    void set_remote_ice_ufrag(const std::string &v) { remote_ice_ufrag_ = v; }

    void set_remote_ice_pwd(const std::string &v) { remote_ice_pwd_ = v; }

    const std::string &get_remote_ice_ufrag() const { return remote_ice_ufrag_; }

    const std::string &get_remote_ice_pwd() const { return remote_ice_pwd_; }
    ...
    std::shared_ptr<DtlsCert> get_dtls_cert() { return dtls_cert_; }

    void set_dtls_cert(std::shared_ptr<DtlsCert> dtls_cert);

    void set_send_socket(UdpSocket *socket) { send_rtp_socket_ = socket; }
    ...
    boost::asio::awaitable<bool> process_dtls_packet(std::unique_ptr<uint8_t[]> data, size_t len,
                                                     UdpSocket *sock,
                                                     const boost::asio::ip::udp::endpoint &remote_ep);
    boost::asio::awaitable<bool> process_srtp_packet(std::unique_ptr<uint8_t[]> data, size_t len,
                                                     UdpSocket *sock,
                                                     const boost::asio::ip::udp::endpoint &remote_ep);
    boost::asio::awaitable<void> async_process_udp_msg(UdpSocket *sock, std::unique_ptr<uint8_t[]> data,
                                                       size_t len, boost::asio::ip::udp::endpoint &remote_ep);
    boost::asio::awaitable<bool> send_udp_msg(const uint8_t *data, size_t len);

public:
    boost::asio::awaitable<bool> process_whip_req(std::shared_ptr<HttpRequest> req,
                                                  std::shared_ptr<HttpResponse> resp);
    boost::asio::awaitable<bool> process_whep_req(std::shared_ptr<HttpRequest> req,
                                                  std::shared_ptr<HttpResponse> resp);
...
private:
    ...
    boost::asio::awaitable<bool> process_stun_binding_req(std::shared_ptr<StunMsg> stun_msg, UdpSocket *sock,
                                                          const boost::asio::ip::udp::endpoint &remote_ep);
    void on_dtls_handshake_done(SRTPProtectionProfile profile, const std::string &srtp_recv_key,
...
private:
    ...
    boost::asio::experimental::concurrent_channel<void(boost::system::error_code, UdpSocket *,
                                                       std::unique_ptr<uint8_t[]>, size_t,
                                                       boost::asio::ip::udp::endpoint)>
        udp_msgs_channel_;

    int64_t last_active_time_ = Utils::get_current_ms();
    boost::asio::steady_timer
        alive_timeout_timer_;  // 超时定时器，如果超过一段时间，没有任何数据，则关闭session
    ...
    boost::asio::experimental::concurrent_channel<void(boost::system::error_code,
                                                       std::vector<std::shared_ptr<RtpPacket>>)>
        send_rtp_pkts_channel_;

    boost::asio::experimental::concurrent_channel<void(boost::system::error_code,
                                                       std::shared_ptr<RtcpFbPacket>)>
        send_rtcp_fb_pkts_channel_;

    boost::asio::experimental::concurrent_channel<void(boost::system::error_code,
                                                       std::shared_ptr<RtcpPacket>)>
        send_rtcp_pkts_channel_;
